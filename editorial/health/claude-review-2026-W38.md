...
- **A Coverage** ✅ — every announced game for the 12 nations to 9 Nov is in the feed with the right tag: PNC final and 3rd place (19 Sep), AUS v RSA Perth (27 Sep), Bledisloe 10/17 Oct, JPN v FIJ (24 Oct), NC round 4 (6–8 Nov, kick-offs match SA Rugby's list). No Six Nations side plays before November.
...
- **E Reminders** ✅ — 282/284 app tests pass against the 14 Sep feed shape; the two failures assert August-specific series legs from the 22 Aug snapshot (date assumptions, not shape). Coverage and dedupe assertions pass. `check:reminders` needs the CDN, unreachable from the sandbox.
- **F Pipelines** ✅ — last 7 days: refresh-data 100/100, digests 23/23, news 47/47, rankings, stats, vendor probe, watchdog, weekly health all green; team-events 1 failure (#77, the race #109 fixed). Crons run ~4.5 h late (01:00 → 05:30 UTC daily); harmless. TestFlight run 29 matches the last app-code commit on main.
...
- Perth kick-off: feed says 09:45 UTC (17:45 AWST); Wallabies' own post says 17:30. Will re-check when ESPN settles it.
- `trc-2026` "announced" with no fixtures: there is no Rugby Championship in 2026, ESPN carries an empty season shell. Not selectable, so cosmetic.
- The two date-pinned reminder tests will keep failing on any refreshed snapshot; worth re-basing on a late-season snapshot once the NC finals are in.
- One Italy v South Africa listing (SA Rugby mag) shows La Nucía, Spain; World Rugby, ESPN and Sky say Turin. Feed says Turin.
